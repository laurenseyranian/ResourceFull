		...
		<p class="title text-center">Keeping Neighbors Safe Where They Are</p>
			...
			<div class="navbar">
				...
				<input type="hidden" name="${_csrf.parameterName}"
					...
					class="btn btn-link text-light mr-4" type="submit" value="LOGOUT" />
				</form>
			</div>
	</div>

	<p class="text-left ml-4 welcome">
		Welcome,
		<c:out value="${currentUser.first_name}"></c:out>! Use the table below to add, view resource needs list for, or update a curbside community.  
	</p>
		...
		<div class="col-6">
			<table class="table ml-2">
				<thead class="thead-dark">
					<tr>
						<th scope="col" class="lead">Community</th>
				...
				<tbody>
					<c:forEach items="${communities}" var="community">
						<tr>
							<td class="tableText">
							<a href="/resourcefull/neighborhood/${community.id}"><c:out
										value="${community.name}" /></a></td>
							<td class="tableText"><c:out value="${community.residents}" /></td>
							<td class="tableText"><c:out value="${community.pets}" /></td>
							<td class="tableText"><a class="btn btn-outline-danger"
								href="/neighborhood/${community.id}/delete">Delete</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
			<a class="btn btn-outline-secondary btn-lg ml-3"
				href="/resourcefull/add/neighborhood">Add a Community</a>
		</div>
