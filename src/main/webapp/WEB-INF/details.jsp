...
<script
	src="https://maps.googleapis.com/maps/api/js?key=AIzaSyD9XuSocOU1HX2gzkpBUWfMxFp6b3uwiVU&callback=initMap"
	async defer></script>
<script>
     class GoogleMap{
            constructor(){
...
<body>
	<div class="topOfPage">
		...
		<div class="navbar">
			<a class="text-light" href="/resourcefull">ABOUT</a>
		</div>
		<div class="navbar">
		...
		<div class="navbar">
			<form id="logoutForm" method="POST" action="/logout">
				<input type="hidden" name="${_csrf.parameterName}"
					value="${_csrf.token}" /> <input class="btn btn-link text-light"
					type="submit" value="LOGOUT" />
			</form>
		</div>
	</div>

	<p class="welcome text-left ml-3">
		Hey
	...
	<p class="contact text-left ml-3">
		Community Liaison Contact Information: <b><c:out
				value="${community.liaisoncontactname}" /> <c:out
				value="${community.liaisoncontactnumber}" /> </b>
	</p>
		...
		<div class="col-6">
			...
			<a class="btn btn-outline-secondary btn-lg ml-3"
				href="/resourcefull/edit/neighborhood/${community.id}">Edit <c:out
					value="${community.name}" /></a> <a
				class="btn btn-outline-secondary btn-lg ml-3"
				href="/resourcefull/home">View all communities</a>
	...
	<div class="messageBox ml-3 mt-2">
		<p class="messageCenter ml-5">Message Center</p>
		<div class="row">
			<div class="col-5 ml-5">
				<form:form method="POST" action="/comment/create"
					...
					<p>
						...
						<form:textarea path="message" class="form-control col-sm-8"
							rows="5" type="text" />
						</textarea>
					</p>
			...
			<div class="col-5 messagesList">
				<div class="box">
					<c:forEach var="message" items="${community.messages}">
						<p class="dinline-block">
						<p class="messageText dinline-block">
							<c:out value="${message.message}" />
						</p> (by <c:out value="${message.user.first_name}" /> on <fmt:formatDate
							pattern="MMMM dd, yyyy" value="${message.createdAt}" />) <a
							class="text-danger ml-4"
							href="/comment/${message.id}/${community.id}/delete">Delete</a>
						</p>
						<p>--------------------------------------------------------------------------------------------------------------</p>
					</c:forEach>
				</div>
